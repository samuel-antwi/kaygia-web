...
const { data: progressData, refresh: refreshProgress } = await useFetch(
  `/api/admin/projects/${props.projectId}/progress`,
  {
    server: false,
  }
);
...
const phases = computed<Record<string, Phase>>(
  () => progressData.value?.phases || {}
);
const phaseProgress = computed<Record<string, PhaseProgress>>(
  () => progressData.value?.phaseProgress || {}
);
...
const createMilestone = async () => {
  ...
  try {
    const response = await $fetch(
      `/api/admin/projects/${props.projectId}/milestones`,
      {
        method: "POST",
        body: {
...
const updateMilestoneStatus = async (milestoneId: string, status: string) => {
  try {
    const response = await $fetch(
      `/api/admin/projects/${props.projectId}/milestones/${milestoneId}`,
      {
        method: "PATCH",
        body: { status },
      }
...
const deleteMilestone = async () => {
  ...
  try {
    const response = await $fetch(
      `/api/admin/projects/${props.projectId}/milestones/${deleteDialog.value.milestoneId}`,
      {
        method: "DELETE",
      }
    );
...
const saveEditedMilestone = async () => {
  ...
  try {
    const response = await $fetch(
      `/api/admin/projects/${props.projectId}/milestones/${editingMilestone.value.id}`,
      {
        method: "PATCH",
        body: {
    ...
    <div class="space-y-4">
      <Alert class="bg-blue-50 border-blue-200">
        ...
        <AlertDescription class="text-blue-700">
          This progress tracking is for internal project management only.
          Clients cannot see these milestones or progress updates.
        </AlertDescription>
      </Alert>
              ...
              >
                <div class="flex justify-between items-center">
                  ...
                  <span class="text-xs text-muted-foreground">
                    {{ phaseProgress[phase.id]?.progress || 0 }}% ({{
                      phaseProgress[phase.id]?.milestones?.filter(
    ...
    <div class="space-y-4">
      ...
      >
        ...
        <p class="text-sm text-muted-foreground mt-1">
          Create milestones to track detailed progress. Client currently sees
          {{ overallProgress }}% based on project status.
        </p>
        <Alert class="mt-4 text-left max-w-2xl mx-auto">
          <AlertCircle class="h-4 w-4" />
          <AlertTitle>Quick Start</AlertTitle>
          <AlertDescription>
            Start with Discovery phase milestones like "Requirements Gathering",
            "Scope Definition", and "Timeline Planning" to show granular
            progress to your client.
          </AlertDescription>
        </Alert>
          ...
          <div v-if="editingMilestone?.id === milestone.id" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="space-y-2">
                ...
                <Input id="edit-name" v-model="editingMilestone.name" />
              </div>
            ...
            <div class="flex justify-end space-x-2">
              ...
              <Button @click="saveEditedMilestone"> Save Changes </Button>
            </div>
          </div>
          ...
          <div v-else class="flex items-start justify-between">
            ...
            <div class="flex items-center space-x-2">
              ...
              <Button
                variant="outline"
                size="sm"
                @click="openDeleteDialog(milestone)"
              >
